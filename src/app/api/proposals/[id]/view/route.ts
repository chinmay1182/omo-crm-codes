...
export async function GET(
  ...
  try {
    ...
    // This allows us to fetch the data first, then apply OUR application-level permissions
    ...
    // Post-fetch permission check (since we need to check assignment on the lead)
    if (agentPermissions) {
    ...
    if (lead?.service) {
      ...
      // First, let's see what services exist in the database
      const { data: allServices } = await client
        .from('services')
        .select('id, service_name, unique_service_code, service_names')
        .limit(10);
    ...
    // "jo services me hai vahi karo" -> Use service breakdown from DB.

    let proposalDiscount = Number(proposal.discount || 0);
    ...
    // Fallback: If no service details found, we might have to use the old reverse logic, 
    // but assuming service is found:
    ...
    if (dbServiceFee > 0 || dbProfessionalFee > 0) {
      ...
      // Use Service Discount if available ("service wale isme" / intent is to use service defaults)
      ...
      finalTotal = parseFloat(proposal.amount ?? "0");
        ...
        <style>
          * {
          ...
          body {
            ...
            color: #333;
            background: #f0f0f0; /* Light gray background for view mode */
            line-height: 1.6;
            display: flex;
            justify-content: center; /* Center the A4 page */
            min-height: 100vh;
            padding: 20px;
          }
          ...
          .proposal-container {
            ...
            box-shadow: 0 0 10px rgba(0,0,0,0.1); /* Add shadow for viewing */
          }
